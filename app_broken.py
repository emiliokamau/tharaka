...
def register():
    ...
    # Validate
    if not all([data.get('full_name'), data.get('username'), data.get('email'), 
                data.get('phone'), data.get('license_number'), data.get('vehicle_type'), 
                data.get('password')]):
    ...
    if Driver.query.filter_by(license_number=data['license_number']).first():
        return jsonify({'success': False, 'message': 'License already registered'}), 409
